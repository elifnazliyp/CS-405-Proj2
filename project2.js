...
class MeshDrawer {
	...
	constructor() {
		 ...
		 */
		this.lightPos = [lightX, lightY, 1.0]; // Light direction
        this.ambientLight = 0.5; // Ambient light intensity
        this.enableLightingFlag = false; // Flag to enable/disable lighting
	...
	draw(trans) {
		 ...
		 */
		gl.uniform1i(this.showTexLoc, this.enableLightingFlag);
		gl.uniform1i(gl.getUniformLocation(this.prog, 'enableLighting'), this.enableLightingFlag);
		gl.uniform1f(gl.getUniformLocation(this.prog, 'ambient'), this.ambientLight);
...
const meshFS = `
    ...
    void main() {
        if (showTex) {
            ...
            // Task 2: Lighting calculations
            if (enableLighting) {
                vec3 ambientColor = ambient * texColor.rgb;

                vec3 lightDir = normalize(lightPos - v_normal); // Direction from fragment to light source
                float diff = max(dot(v_normal, lightDir), 0.0); // Diffuse reflection
